...
func InicializarParticion(tamanios []int) {
	...
	for i, tamPart := range tamanios {
		// Verificar si la partición excede el tamaño total de memoria
		...
		particion := &entity.Particion{
			Base:   base,
			Limite: base + tamPart, // Calcular el límite correctamente
			Libre:  true,
			PID:    0,
		}
		...
		slog.Debug("Partición creada ->", "Particion", i, "Base:", particion.Base, "Límite:", particion.Limite)
...
func LiberarParticion(pid uint32) {
	...
	for i, p := range entity.Particiones {
		if !p.Libre && p.PID == pid {
			...
			p.Libre = true
			p.PID = 0
			if config.GetInstance().Scheme == "DINAMICAS" {
