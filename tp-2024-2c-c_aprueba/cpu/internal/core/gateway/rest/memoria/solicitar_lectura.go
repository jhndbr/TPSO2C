...
func EnviarSolicitudLeerValorMemoria(direccionFisica uint32,pid uint32,tid uint32) (*uint32, error) {

	conf := config.GetInstance()

	url := fmt.Sprintf("http://%s:%d/memory/leerMemoria?direccion=%d&pid=%d&tid=%d", conf.IPMemory, conf.PortMemory, direccionFisica, pid, tid)
