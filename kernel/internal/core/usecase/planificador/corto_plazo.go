...
func AtenderMutexLock(mutexID string) {
	...
	if mutex.Estado == 0 {
		...
		repository.AgregarHiloBloqueadoPorMutex(hiloEnEjecucion, mutexID, procesoEnEjecucion)
...
func AtenderMutexUnlock(mutexID string) {
	...
	var hi = repository.PopHiloBloqueadoPorMutex(mutexID, procesoEnEjecucion)
	if hi != nil {
		hilos.Add(hi)
		repository.DesbloquearHilosSegunAlgoritmo(hilos)
		repository.AddPrioridadEnOrden(hi.Priority)
		slog.Info("Mutex en uso", "Mutex:", mutex.Nombre, "TID:", hi.TID)
		_ = semReady.Post()
...
func ProcessIOQueue() {
	for request := range ioQueue {
		slog.Info("Solicitud de I/O recibida", "PID:", request.TCB.PID, "TID:", request.TCB.TID)
		wg.Add(1)
