...
func Execute(ctx *dto.Context) {
	...
	case "READ_MEM":
		if !huboSegmentation {
			...
			valor, err := memoria.EnviarSolicitudLeerValorMemoria(DireccionFisica, ctx.PID, ctx.TID)
			if err != nil {
				return
			}
	...
	case "WRITE_MEM":
		if !huboSegmentation {
			...
			err := memoria.EnviarSolicitudEscribirEnMemoria(DireccionFisica, valor, ctx.PID, ctx.TID)
			if err != nil {
				return
			}
		} else {
	...
	//slog.Info("##","pidINTERRUPT:",dto.PidInterrupt,"tidINTERRUPT:",dto.TidInterrupt,"pid:",ctx.PID,"tid:",ctx.TID)
	if dto.PidInterrupt == ctx.PID && dto.TidInterrupt == ctx.TID {
		slog.Info("Desalojando hilo", "PID", ctx.PID, "TID", ctx.TID)
		err := memoria.ActualizarContextoDeEjecucion(ctx)
